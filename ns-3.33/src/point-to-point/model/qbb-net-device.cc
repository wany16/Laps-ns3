  ...
  std::unordered_map<int32_t, Time> RdmaEgressQueue::cumulative_pause_time;
   ...
   bool QbbNetDevice::IsPfcEnabled() const {
	...
	QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		{													
			int qIndex = m_rdmaEQ->GetNextQindex(m_paused); // the index of the qp (NOT queue or virtual queue) to send next, // for netdevice in host, has 8 virtual queues
			if (qIndex != QINDEX_OF_NONE_PACKET_IN_SERVER) // exist packet to send
			{ 
				if (qIndex == int(QINDEX_OF_ACK_PACKET_IN_SERVER))
				{																 
					p = m_rdmaEQ->DequeueQindex(QINDEX_OF_ACK_PACKET_IN_SERVER); // get the actual packet to send, exist ack packets in the highest priority queue
