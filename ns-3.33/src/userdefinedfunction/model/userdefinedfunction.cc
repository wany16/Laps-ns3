...
void install_routing_entries_based_on_single_smt_entry_for_laps(NodeContainer nodes, std::map<Ipv4Address, uint32_t> &ip2nodeId)
{
  ...
  void install_routing_entries(global_variable_t *varMap) {
    ...
    for (size_t i = 0; i < varMap->svNodes.GetN(); i++)
    {
      Ptr<Node> srcNode = varMap->svNodes.Get(i);
      NS_ASSERT_MSG(srcNode->GetNodeType() == SERVER_NODE_TYPE, "Error in srcNode type");
      for (size_t j = 0; j < varMap->svNodes.GetN(); j++)
      {
        if (i == j)
        {
          continue;
        }
        Ptr<Node> dstNode = varMap->svNodes.Get(j);
        NS_ASSERT_MSG(dstNode->GetNodeType() == SERVER_NODE_TYPE, "Error in dstNode type");
        Ipv4Address dstAddr = dstNode->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal();
        Ptr<RdmaDriver> rdmaDriver = srcNode->GetObject<RdmaDriver>();
        NS_ASSERT_MSG(rdmaDriver, "unfound rdma driver on server node");
        rdmaDriver->m_rdma->AddTableEntry(dstAddr, 1);
      }
    }
